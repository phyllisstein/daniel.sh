...
const getBoxFor = R.curry((map, z) => {
  const values = map[z]
  return values.join(' ')
})

const getTextFor = R.curry((map, z) => {
  const values = R.dropLast(1, map[z])
  return values.join(' ')
})
...
const boxen = R.times(getBoxShadow)
const texten = R.times(getTextShadow)
